...
public class CleanClientSubscriberDncRecord implements Module {
  ...
  private CleanClientSubscriberDncRecordConf ccsxrConf;
  ...
  public CleanClientSubscriberDncRecord( SubscriberApp app , SubscriberConf conf ) {
    initialized = false;
    ...
    this.app = app;
    this.conf = conf;
    this.ccsxrConf = conf.getCleanClientSubscriberDncRecordConf();
    if ( ccsxrConf == null ) {
      DLog.warning( lctx , "Failed to initialized "
          + ", found null clean client subscriber dnc record conf" );
      return;
    }
    ...
    // setup worker thread
    workerThread = new CleanClientSubscriberDncRecordThread();
    activeThread = false;
  ...
  private class CleanClientSubscriberDncRecordThread extends Thread {

    public CleanClientSubscriberDncRecordThread() {
      super( "CleanClientSubscriberDncRecordThread" );
    }
        ...
        if ( counter % ( delay1s * ccsxrConf.getSleepInitializedInSeconds() ) == 0 ) {
          try {
            ...
            if ( ccsxrConf.getLimitRecords() > 0 ) {
              listGrpItems = dao
                  .listGroupDuplicatedActiveNumbersForDnc( ccsxrConf
                      .getLimitRecords() );
            ...
            if ( ( listGrpItems != null ) && ( listGrpItems.size() > 0 ) ) {
              listSelItems = dao
                  .listDuplicatedActiveNumbersForDnc( listGrpItems );
            ...
            if ( ( listSelItems != null ) && ( listSelItems.size() > 0 ) ) {
              listDelItems = dao.listDeletedNumbersForDnc( listSelItems );
